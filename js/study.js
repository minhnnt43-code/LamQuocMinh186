...
export const initStudyModule = (data, user) => {
    ...
    const draftTitleInput = document.getElementById('draft-main-title-input');
...
const handleDeleteAchievementInModal = async () => {
    ...
    const index = globalData.achievements.findIndex(a => a.id === id);
    if (index > -1) {
        // [BẢO VỆ] Backup thành tích trước khi xóa vào localStorage
        ...
        console.log(`📦 Đã backup thành tích "${deletedAch.name || deletedAch.title}" vào ${backupKey}`);

        globalData.achievements.splice(index, 1);

        // [BẢO VỆ] Nếu còn achievements thì mới lưu, tránh ghi đè rỗng
        if (globalData.achievements.length > 0) {
            await saveUserData(currentUser.uid, { achievements: globalData.achievements });
        } else {
            // Nếu xóa hết, dùng deleteField thay vì ghi mảng rỗng
            console.warn('⚠️ Đã xóa thành tích cuối cùng');
            await saveUserData(currentUser.uid, { achievements: [] }); // Sẽ bị block bởi saveUserData
        }

        renderAchievements();
        closeModal('achievement-modal');
        showNotification('Đã xóa thành tích (backup trong localStorage)', 'success');
    }
};

// [BẢO VỆ] Hàm khôi phục thành tích từ backup (gọi từ Console: window.restoreAchievementBackup())
window.restoreAchievementBackup = async () => {
    ...
    if (backupKeys.length === 0) {
        console.log('❌ Không có backup thành tích nào trong localStorage');
        return;
    }

    console.log(`📦 Tìm thấy ${backupKeys.length} backup thành tích:`);
    ...
    console.log('');
    console.log('Để khôi phục, gọi: window.restoreAchievementByKey("achievement_backup_xxxxx")');
};

window.restoreAchievementByKey = async (key) => {
    const data = localStorage.getItem(key);
    if (!data) {
        console.log('❌ Không tìm thấy backup với key:', key);
        return;
    }
    ...
    if (exists) {
        console.log('⚠️ Thành tích này đã tồn tại trong danh sách');
        return;
    }
    ...
    // Xóa backup sau khi khôi phục thành công
    localStorage.removeItem(key);
    console.log(`✅ Đã khôi phục thành tích "${ach.name || ach.title}" và xóa backup`);
    showNotification(`Đã khôi phục thành tích: ${ach.name || ach.title}`, 'success');
};
...
const loadDraft = (draft) => {
    ...
    const titleInput = document.getElementById('draft-main-title-input');
