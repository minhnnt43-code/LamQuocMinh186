...
export const saveUserData = async (uid, data, options = {}) => {
    try {
        ...
        // [BẢO VỆ] Ngăn ghi đè mảng rỗng lên dữ liệu quan trọng
        // Trừ khi forceOverwrite = true (VD: xoá tất cả có chủ đích)
        const protectedFields = ['achievements', 'tasks', 'documents', 'projects', 'drafts', 'outlines'];
        const safeData = { ...data };

        if (!options.forceOverwrite) {
            for (const field of protectedFields) {
                // Nếu cố ghi mảng rỗng, bỏ qua field đó (giữ nguyên data cũ)
                if (Array.isArray(safeData[field]) && safeData[field].length === 0) {
                    console.warn(`⚠️ [Bảo vệ dữ liệu] Bỏ qua ghi đè mảng rỗng cho: ${field}`);
                    delete safeData[field]; // Xóa khỏi data để không ghi đè
                }
            }
        }

        await setDoc(docRef, { ...safeData, lastUpdated: new Date().toISOString() }, { merge: true });
    } catch (error) {
