...
const handleViewChange = (view) => {
    ...
    switch (view) {
        ...
        case 'timeline':
            ...
            break;
        case 'timeblock':
            // Show Time Blocking view, hide regular calendar
            container.style.display = 'none';
            const tbView = document.getElementById('time-blocking-view');
            if (tbView) {
                tbView.style.display = 'flex';
                import('./time-blocking.js').then(m => {
                    m.initTimeBlocking(globalData, currentUser);
                    m.renderTimeBlockingView();
                });
            }
            return; // Early return to skip hiding tbView below
        case 'week':
        default:
...
const switchView = (view) => {
    ...
    // Always hide time-blocking view first (unless switching to it)
    const tbView = document.getElementById('time-blocking-view');
    if (tbView && view !== 'timeblock') {
        tbView.style.display = 'none';
        ...
        case 'timeblock':
            ...
            if (tbView) {
                ...
                import('./time-blocking.js').then(m => {
                    ...
                    console.error('Time Blocking module error:', err);
                    tbView.innerHTML = `<div style="padding:50px;text-align:center;color:#6b7280;">🧩 Time Block - Đang tải...</div>`;
                });
            }
            break;
        case 'week':
