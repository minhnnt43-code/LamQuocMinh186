...
export function setupNavigation() {
    ...
    buttons.forEach(btn => {
        btn.addEventListener('click', (e) => {
            ...
            // [MỚI] Init các modules đặc biệt khi section được active
            if (targetId === 'eisenhower') {
                import('./eisenhower.js').then(m => {
                    if (window.globalData) {
                        m.initEisenhower(window.globalData, window.currentUser);
                        m.renderEisenhowerMatrix();
                    }
                }).catch(err => console.error('Eisenhower module error:', err));
            }
